...
To supply vector arguments, just find the codes you want and supply them to `imf_data` using the parameter name as the argument name. The example below shows how to request annual coal price data from the Primary Commodity Price System database:
...
params$commodity <- params$commodity %>%
    filter(str_detect(tolower(.$description),"coal"))

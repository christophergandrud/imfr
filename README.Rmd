...
* Submit to CRAN
* Call to `http://dataservices.imf.org/REST/SDMX_JSON.svc/GenericMetadata/%database_id%` in `imf_parameters` to get all parameter names and values with a single API call (and thus reduce the need for throttling to avoid hitting the API rate limit.)